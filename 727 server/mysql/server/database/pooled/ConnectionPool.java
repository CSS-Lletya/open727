...
public class ConnectionPool implements ConnectionListener {
	...
	private ConcurrentLinkedQueue<DatabaseConnection> databaseConnections = new ConcurrentLinkedQueue<DatabaseConnection>();
	private ConcurrentLinkedQueue<DatabaseConnection> priorityConnections = new ConcurrentLinkedQueue<DatabaseConnection>();
		...
		synchronized (this) {
			...
			if (!hasSpace && priorityConnection) {
				...
				if (priorityConnections.size() < connectionSettings.getPriorityConnections()) {
					DatabaseConnection databaseConnection = addConnection(true, true);
					databaseConnection.setConnectionHash(connectionHash);
					databaseConnection.assureConnection();
					databaseConnection.setPriorityConnection(true);
					databaseConnection.setBusy(true);
	...
	/**
	 * Release connections that have exceeded expected life time.
	 */
	...
	public String toString() {
		...
		debug.append("Connections in the pool: " + poolSize);
		debug.append("\n");
		...
		debug.append("Busy connections: " + +busyConnections);
		debug.append("\n");
		debug.append("Active connections: " + connectionHashes.toString());
		debug.append("\n");
		debug.append("Opened connections: " + openedConnections);
		debug.append("\n");
		debug.append("Closed connections: " + (poolSize - openedConnections));
		debug.append("\n");
		debug.append("Connection requests: " + (connectionRequests));
