...
public class RuneWikiBonusDumper {
	...
	public static final void main(String[] args) throws IOException {
		...
		for (int itemId = 22067; itemId < Utils.getItemDefinitionsSize(); itemId++) {
			if (ItemDefinitions.getItemDefinitions(itemId).isWearItem()
				...
				if (dumpItem(itemId))
					System.out.println(
							"Dumped ITEM: " + itemId + ", " + ItemDefinitions.getItemDefinitions(itemId).getName());
		}
	}
	...
	public static boolean dumpItem(int itemId) {
		...
		if (file.exists())
			return false;
		String pageName = ItemDefinitions.getItemDefinitions(itemId).getName().replace(" (black)", "")
				.replace(" (white)", "").replace(" (yellow)", "").replace(" (red)", "");
				...
				if (bonusId == 0 || bonusId == 5) {
					...
					if (line.startsWith(replace)) {
						line = line.replace(replace, "").replaceAll(" ", "").replace("%", "").replace("+", "")
								.replace(".0", "").replace("52to70", "70").replace("0(4trimmed)", "4")
								.replace("15(Slayertasksonly)", "0").replace(".5", "").replace("?", "0")
