...
public class RuneWikiNPCDefinitionsDumper {
	...
	public static boolean dumpNPC(int npcId) {
		...
		try {
			...
			for (int linesCount = 0; linesCount < page.getLines().size(); linesCount++) {
				...
				if (line.equalsIgnoreCase(COMBAT_LEVEL_LINE)) {
					...
					if (attackSpeedLine.contains("Unknown"))
						...
						String speed = attackSpeedLine.substring(attackSpeedLine.indexOf("Speed") + 5,
			...
			try {
				...
				writer.newLine();
				writer.write(npcId + " - " + hitpoints + " -1 -1 -1 " + attackSpeed + " 1 60 " + maxhit + " " + t
						+ " -1 -1 " + a);
				writer.newLine();
				writer.flush();
				writer.close();
	...
	public static String getFormatedString(String s) {
		return s.replace("</th><td>", "").replace("to", ",").replace("-", ",").replace("/", ",").replace("Varies", "0")
				.replaceAll(" ", "").replace("<br", "")
				.replace("<br/>(0in<ahref=\"/wiki/Daemonheim\"title=\"Daemonheim\">Daemonheim</a>)", "")
				.replace(
						"(13in<i><ahref=\"/wiki/The_Restless_Ghost\"title=\"TheRestlessGhost\">TheRestlessGhost</a></i>)",
						"13,")
				.replace("(variesinDaemonheim:...", "");
