...
public class WikiEqupSlotDumper {
	...
	private static EquipSlot getEquipSlot(Item item, String line) {
		...
		if (line.equals("Neck slot")) {
			...
			/*
			 * if (Equipment.isFullHat(item)) new EquipSlot(id, SlotType.FULL_HELMET); else if (Equipment.isFullMask(item)) return new EquipSlot(id, SlotType.FULL_MASK);
			 */
	...
	public static ArrayList<String> getPage(Item item) {
		try {
			String pageName = item.getDefinitions().getName().replace(" (black)", "").replace(" (white)", "")
